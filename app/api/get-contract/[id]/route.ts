...
export async function GET(request: NextRequest) {
    ...
    try {
        const resp = await fetch("http://localhost:5251/api/v1/Contract/get/"+id, {
            method: "get",
            headers: headers
        })
        // Handle the response if needed
        if (resp.ok) {
            const res: GeneralResponseGeneric<ContractModel> = await resp.json() as GeneralResponseGeneric<ContractModel>
